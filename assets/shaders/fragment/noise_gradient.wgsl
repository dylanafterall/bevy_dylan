...
fn fragment(in: VertexOutput,) -> @location(0) vec4<f32> {
    ...
    uv += t;    // animate/move shader

    var f = 0.0;
    // left: noise
    if(uv.x < 0.6) {
        f = gradient_noise(32.0 * uv);
    // right: fractal noise (4 octaves)
    } else {
        uv *= 8.0;
        let m = mat2x2<f32>(1.6, 1.2, -1.2, 1.6);
        f  = 0.5000 * gradient_noise(uv);
        uv = m * uv;
        f += 0.2500 * gradient_noise(uv);
        uv = m * uv;
        f += 0.1250 * gradient_noise(uv);
        uv = m * uv;
        f += 0.0625 * gradient_noise(uv);
        uv = m * uv;
    }
    f = 0.5 + 0.5 * f;
